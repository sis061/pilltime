...
export default function ProfileDrawer({
  ...
  return (
    ...
    >
      <DrawerContent className="!p-4 bg-slate-100 h-[100dvh] w-[160px] !ml-auto rounded-tr-none rounded-bl-[10px]">
        ...
        <div className="font-bold !text-pilltime-blue text-sm !p-2 flex gap-2 !mt-2">
          <UserCog className="h-5 w-5 !mr-1" color="#3B82F6" />
          프로필 관리
        </div>
        ...
        <div className="flex flex-col w-full items-start !pl-2">
          {menuButtons.map(({ key, label, iconLeft: Icon, onClick }) => (
            ...
            >
              {Icon ? <Icon className="!mr-2 h-5 w-5" color="#3B82F6" /> : null}
              {label}
            </Button>
          ))}
        </div>
